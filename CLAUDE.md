...
- After making changes, commit them.  Do not push
...
| Run the app             | `make run` (debug build, runs `hackernews_tim` crate)    |
| Type-check everything   | `make check`                                             |
| Run tests               | `make test` (workspace)                                  |
| Single test             | `cargo test -p <crate> <test_name>` (e.g. `-p hackernews_tim`) |
...
`cargo run -p hackernews_tim -- -c path/to/config.toml -a path/to/hn-auth.toml -l /tmp -i <item_id>`
...
When bumping versions, both `config_parser2` and `config_parser_derive` publish to crates.io independently; the `hackernews_tim` crate references `config_parser2` by both version and path, so local changes are picked up without a publish.

## Architecture

The application is a Cursive TUI that layers custom views on top of Hacker News data fetched from two upstream APIs.

**Entry flow (`hackernews_tim/src/main.rs`)**: `main` → `init_app_dirs` (uses `dirs_next`, returns the app-specific `hackernews-tim` subdirectory under the user's config dir plus a list of legacy dirs — currently `$XDG_CONFIG_HOME` and `$HOME/.config` — where pre-subdir files lived) → `parse_args` (clap, flags `-c/-a/-l/-i/--init-config/--update-theme`; `--init-config` and `--update-theme` are mutually exclusive) → `init_logging` (tracing + `RUST_LOG`, default `hackernews_tim=info`, writes `hn-tui.log`) → unless `--init-config` was given, call `config::migrate_legacy_file` for the config and auth files when the user didn't override the path: if the new default location is empty but a legacy copy exists, copy it in (original preserved). The config migration also looks for a legacy `hn-tui.toml` (the pre-rename name) inside the app subdir itself, in addition to the pre-subdir legacy dirs → if `--init-config <light|dark>` was given, write the embedded default to the resolved `--config` path and exit → else if `--update-theme <light|dark>` was given, replace just the `[theme]` table in the existing `--config` file via `config::update_theme_in_place` (errors if the file is missing) and exit → else if the config file is missing and stdin/stdout are both TTYs, prompt via `config::prompt_for_flavor` to write one → `config::load_config` (populates a global) → `init_auth`, which either reads `hn-auth.toml` or prompts via `config::prompt_for_auth` on first run → `run`, which builds an `HNClient`, optionally logs in, calls `view::init_ui`, then wraps the backend in `cursive_buffered_backend` to avoid flicker (see gyscos/Cursive#142).
...
**Config (`src/config/`)**: `Config` derives `ConfigParse` from the workspace's own `config_parser2` crate, so every field is optional in TOML and merged over `Config::default()`. Submodules: `theme.rs` (palette + styles, consumed in `view::init_ui` to patch the Cursive palette), `keybindings.rs` (typed `KeyMap` with a global section, per-view sections, and user-defined `custom_keymaps` for extra story tags), `init.rs` (embeds `examples/config.toml` and `examples/config-dark.toml` via `include_str!`, exposes the `ConfigFlavor` enum, `write_default_config`, and `update_theme_in_place` — which swaps the `[theme]` table of an existing config using `toml_edit` — plus the TTY-gated first-run prompts `prompt_for_flavor` and `prompt_for_auth`). Globals are accessed via `config::get_config()`, `config::get_config_theme()`, `config::get_global_keymap()`. Defaults live in `examples/config.toml` (light) and `examples/config-dark.toml` (dark); docs live in `docs/config.md`.
